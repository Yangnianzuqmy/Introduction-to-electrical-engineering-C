    ...
    def indexDocs(self, root, writer):
        ...
        while line:
            try:
                i+=1
                tmp = line.split('\t')
                img_title.append(tmp[0])
                img_url.append(tmp[1])
                title.append(tmp[3])
                page_url_list.append(tmp[4])
                line = file.readline()
            except:
                i+=1
                print i
        ...
        for root, dirnames, filenames in os.walk(root):  # 遍历testfolder下的文件
            for i in range(len(img_title)):
                print "adding",img_url[i]
                try:
                    path = os.path.join(root, valid_filename(img_url[i])+'.txt')
                    file = open(path)
                    contents = unicode(file.read(), 'utf8')  # 将文件转为unicode再处理，假设原doc编码为GBK，并将内容存放在contents中
                    file.close()
                    doc = Document()
                    doc.add(Field("img_url", img_url[i], t1))
                    doc.add(Field("page_url", page_url_list[i], t1))
                    doc.add(Field("title", title[i], t1))
                    if len(contents) > 0:
...
if __name__ == '__main__':
    ...
    IndexFiles('lab5_html_jieba', "index")
    end = datetime.now()
    print end - start
